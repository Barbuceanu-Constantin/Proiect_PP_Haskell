...
{-
    Am modificat read2 cu read1. read2 se foloseste pentru cerinta de la etapa 1, read2 pentru cerinta de la etapa3.
    Asa era inainte: fromIntegral $ sum $ map read2 list
    Asa e acum: sum $ map read1 list
-}
totalOfMinutes :: [String] -> Float
totalOfMinutes list = sum $ map read1 list
...
takeColumns :: [Int] -> Table -> Table
takeColumns (x:[]) t = map (\string -> [string]) (extractColumn t x)
takeColumns (x:xs) t = hunion (map (\string -> [string]) (extractColumn t x)) (takeColumns xs t)

instance Eval Query where
    eval (FromTable t) = Table t
    eval (AsList colname (FromTable t)) = List $ map (!!index) (tail t)
                            where index = getIndex colname (head t) 0
    eval (Sort colname (FromTable t)) = Table $ tsort colname t
    eval (ValueMap op (FromTable t)) = Table $ map (map op) t
    ...
    eval (Projection colnames (FromTable t1)) = Table $ takeColumns listOfIndexes t1
                                        where listOfIndexes = getIndexes colnames (head t1)
